...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/match_names \
  -H 'Content-Type: application/json' \
  -d '{"names":["Quercus alba","Panthera leo"]}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/match_names`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "opentreeoflife": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-opentreeoflife"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-opentreeoflife
```
